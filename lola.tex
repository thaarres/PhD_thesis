...
\end{figure}
\clearpage
% \chapter{What \textit{does} LoLa learn?}
% \label{sec:lola:learn}
% As a first study of what features LoLa learns, one can remove features in the LoLa and Cola layers. The first thing to check
%
% Compare nominal training to training after removing variables sensitive to mass and pT
% Remove CoLa column that passes  sum of all 4-momentum (4 vector of “full jet”)
% not much impact on overall performance
% Remove Lola mass and pT  variables reduce performance significantly
% worst when removing Σpμ and m+pT (like removing jet mass/pT)
\chapter{Summary}
