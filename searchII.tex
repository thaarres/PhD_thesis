...
Finally, we require at least one AK R=0.8 jet in the event with a momentum greater than 200 \GeV and a softdrop jet mass greater than 40 \GeV. This will be our hadronic \PW candidate.
After reconstructing and selecting all our objects, a set of angular selections are applied in order to increase the purity of real merged \PW bosons. These are the following:
\begin{itemize}
...
\par A summary of the final selection criteria is presented in Table~\ref{tab:searchII:cutsummary}.
...
Muon $\PT$ & $\PT > 53 \GeV$ & \\
Electron $\eta$ & $|\eta|_{\text{SC}} <2.5$ except [1.4442, 1.566] range & Exclude ECAL barrel-endcap transition region.\\
Muon $\eta$  & $|\eta|<2.1$  & \\
\hline
...
\cline{1-1}
PUPPI softdrop jet mass & $ 40 < m_{sd} < 150 \GeV$ &  \\
