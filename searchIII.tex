...
\begin{equation}
  ...
  \end{equation}
where $\sigma_{p_{T,\textrm{Bin}=i}}$ is the uncertainty on the scale factor on \PT bin $i$. In contrast with what was found for the \nsubj-based tagger in Section~\ref{sec:searchI:wtagsystematic}, where this uncertainty grew logarithmically with \PT, we find that the corresponding double ratio stabilizes around 1 \TeV in the high-purity category when using \ddt, and it can be approximated as a flat $28\%$. The uncertainty for the low-purity category is still well described by a logarithmic function. The uncertainty $\sigma_{p_{T}}$ and resulting parametrization is shown in Figure~\ref{fig:searchIII:tau21ptdep}.
 \begin{figure}[h!]
 \centering
