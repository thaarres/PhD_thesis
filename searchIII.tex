...
Finally, as we do not know a priori whether Nature behaves more like \PYTHIA{8}, \HERWIG{++}, or \MADGRAPH{}+\PYTHIA{8}, we account for differences due to MC generation and parton-shower modeling by adding three additional shapes corresponding to the three-dimensional templates obtained using different QCD MC generators. Those five shape uncertainties are assigned very large pre-fit values (allowed to float within 33\%), effectively allowing the simulation to take any value to describe the data. The alternate shapes described above are shown in Figure~\ref{fig:searchIII:sys}, here for the 2017 analysis.
\begin{figure}[h!]
\centering
