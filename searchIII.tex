...
\caption{QCD Monte Carlo predictions differ significantly between generators. Since we do not know a-priori whether Nature behaves more like \PYTHIA{8}, \HERWIG{++}, \MADGRAPH{}+\PYTHIA{8} or \POWHEG{}, we allow the 3D template to assume the shape predicted by any of them by adding three additional shapes to the fit, corresponding to the 3D templates obtained using different QCD MC generators.}
...
\label{tab:GOFs}
\end{table}
The post-fit distributions in the data control region using all alternative shapes are shown in Figures~\ref{fig:postfitLPLP_X}--\ref{fig:postfitLPLP_Z} for different jet-mass and dijet invariant mass bins.
\begin{figure}[h!]
\centering
...
\includegraphics[width=0.45\textwidth]{figures/analysis/search3/AN-17-303/limits/compareLimits_Zprime_3D.png}
...
We also evaluate the impacts of each nuisance parameter $\theta$ on the signal strength $r$. This is defined as the shift, $\Delta r$, that is induced when $\theta$ is fixed and brought to its +1$\sigma$ or -1$\sigma$ post-fit values, with all other parameters profiled as normal.
