...
In addition to the observation of a 3.4 $\sigma$ excess in the search for diboson resonances in the all-hadronic final state by ATLAS~\cite{Aad2015}, several little bumps near 2 TeV were observed in other datasets. These were not statistically significant, but rather elusive enhancements, as illustrated by the collection of dijet invariant mass distributions observed by ATLAS and CMS in Figure~\ref{fig:searchIII:bumps}.
\begin{figure}[h!] 
    \centering
    ...
    \includegraphics[height=0.3\textwidth]{figures/analysis/search3/misc/bumps2.pdf}\\
    \includegraphics[height=0.3\textwidth]{figures/analysis/search3/misc/bumps1.pdf}\\
...
\end{figure}
Due to their small size and the way the excesses appeared to slightly shift around, these were obviously not diboson resonances. However, a proposal was made that they could be caused by the observation of the tail of a non-SM boson with a mass slightly different from that of a SM vector boson, as illustrated in Figure~\ref{fig:searchIII:tails}.
\begin{figure}[ht] 
    \centering
    \includegraphics[width=0.3\textwidth]{figures/analysis/search3/misc/tails.png}
    \caption{The small excesses in diboson searches could be caused by the observation of the tail of a non-SM object, peaking at a groomed jet mass slightly higher or lower than that of a W/Z boson.}
...
In order to efficiently search for such types of signals, or any signal peaking in the softdrop jet mass spectrum, we decided to build a generic framework that would allow searching for peaks anywhere in the groomed mass and dijet invariant mass spectrum.
Rather than selecting jets with a groomed mass between 65 and 105 \GeV and then searching for resonances peaking in the dijet invariant mass, we would attempt to look for resonances peaking anywhere in the three dimensional space formed by the groomed mass of each jet and their invariant mass, scanning the full groomed mass spectrum in one analysis. We would first demonstrate the method through the VV all-hadronic analysis, which is the paper introduced here.

\section{Analysis strategy}
The background estimation used in Search I and Search II rely on a one dimensional fit of the dijet invariant mass signal region after a tight jet mass selection has been applied. We now take advantage of the fact that the signal peaks in three dimensions; dijet invariant mass ($\MVV$) and the jet groomed mass of jet 1 and jet 2 ($\MJO$ and $\MJT$), and attempt to extract the signal from the three dimensional $\MVV$-$\MJO$-$\MJT$ plane.
\begin{figure}[h!] 
    \centering
...
\end{figure}
The benefits of doing so is that we can search for resonances decaying to WW, WZ, ZZ, WH or XX in the same analysis.
Additionally, a jet mass selection is no longer needed as we fit the full jet mass line-shape to extract the signal. This effectively increases the signal statistics as a large fraction of the W and Z signal falls outside the mass window. Fitting the jet groomed mass and resonance mass together also allows for the addition of nuisance parameters that simultaneously affect the jet groomed mass and the resonance mass, fully accounting for the correlation between the variables. Finally, we would model the background starting from simulation, rather than from a dijet fit to data. This would allow for the modeling of non-smooth distributions, and could allow the search to go even lower in dijet invariant mass, something we will discuss further in Section~\ref{sec:searchIII:trigger}. Replacing the parametric fit by a simulation-based model, would also reduce the fit sensitivity to background fluctuations in the extreme tails of the dijet invariant mass spectrum. \par
This chapter presents a novel three-dimensional fit method in the search for diboson resonances in the all-hadronic final state, and is based on 77.3 \fbinv of data collected in 2016 and 2017. The method allows to search for VV, VH and HH resonances in one common framework, and could also be used to search for any signal with a final state peaking in the jet groomed mass spectrum.
...
Each analysis has built on findings and improvements that came with the analysis before it: The substructure triggers and mass corrected softdrop jet mass are both used for the 3D fit; the early discovery of the softdrop signal efficiency dependence on \PT led us to derive corrections for it; and the observed lack of constraint on the fit in the dijet invariant mass tail when statistics were low, led us to explore alternate methods starting from simulation when building up the 3D fit.
