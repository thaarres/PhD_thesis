...
The benefits of this procedure, was that it would allow us to scan the full groomed mass spectrum in one analysis. We would first demonstrate the method throught the VV all-hadronic analysis, which is the paper introduced here.
...
\caption{The interpolated Crystal-ball parameters for the dijet invariant mass as a function of $M_X$. The small variations for ALPHA2 have been shown to have no effect on the overall modelling.}
...
This is done separately for $\MJO$ and $\MJT$. The fitted parameters and interpolations are shown in Figure~\ref{fig:MJet1ShapeParam} for \MJO, the corresponding ditributions for \MJT are in Appendix~\ref{app:search3:sigfits}.
\begin{figure}[htpb]
\centering
...
The following fit range and binning is used for the three axes: \MJO/\MJT is fitted from 55 to 215 GeV using 2 GeV bins. $\MVV$ is fitted from 1126 to 5500 GeV. The lower bound is chosen such that to avoid complications in the fitting procedure due to trigger turn-on effects, while the upper bound is chosen considering the highest dijet invariant mass found in data as well as avoiding mis-reconstruction effects at very large $m_{jj}$ and low jet masses. For $\MVV$, the "dijet binning" is used. This binning corresponds to the actual dijet mass resolution and is as follows:

1126,1181,1246,1313,1383,1455,1530,1607,1687,1770,1856,1945,2037,2132,2231,2332,2438,2546,\\
2659,2775,2895,3019,3147,3279,3416,3558,3704,3854,4010,4171,4337,4509,4686,4869,5058,5253,5500
...
As the HPHP category is very limited in statistics, we rather build this template starting from the HPLP templates. This is done by fitting the HPLP 3D PDF to QCD MC in the HPHP category. Figure~\ref{fig:3DkernelsHPLPpythia} and~\ref{fig:3DkernelsHPHPpythia} show the final templates (solid lines) together with the QCD MC (data points) derived from the 2017 MC in the HPLP and HPHP category, respectively. Good agreement between QCD MC and templates in all three dimensions is observed, within statistical uncertainties. The corresponding distributions in 2016 MC can be found in Appendix~\ref{app:search3:2016kernels}.\par
\begin{figure}[p]
\centering
...
% \label{sec:resbkgd}
% In addition to the main background from QCD multijet processes, there are a few sub-dominant backgrounds to consider containing one real vector boson and at least one QCD-jet. These sub-dominant backgrounds consist of W+jets, Z+jets and events from \ttbar processes. As these backgrounds contain a real vector boson, they are resonant around the W/Z mass in the two softdrop jet mass dimensions. The treatment of these therefore differs to that of the non-resonant QCD background. Figure \ref{fig:stack_res_bkg} shows projections on all mass dimensions of the three background contributions. Since the jets in this analysis are randomly sorted, each jet mass dimension contains two contributions: A resonant part made out of real vector boson jets, clearly peaking around the V mass, and a non-resonant part composed of jets originating from a quark/gluon, similar to the QCD multijets background.
% \begin{figure}[h!]
% \includegraphics[width=0.33\textwidth]{figures/analysis/search3/AN-17-303/resonantBkg/CP_background_px_xrange55-215_yrange55-215zrange1126-5000_HPLP.pdf}
% \includegraphics[width=0.33\textwidth]{figures/analysis/search3/AN-17-303/resonantBkg/CP_background_py_xrange55-215_yrange55-215zrange1126-5000_HPLP.pdf}
% \includegraphics[width=0.33\textwidth]{figures/analysis/search3/AN-17-303/resonantBkg/CP_background_pz_xrange55-215_yrange55-215zrange1126-5000_HPLP.pdf}
% \caption{Projections of the sub-dominant backgrounds on the jet mass \MJO (top left) and jet mass \MJT (top right) as well as a projection on the dijet invariant mass \MVV (bottom). All projections shown are using the low purity selections.}
% \label{fig:stack_res_bkg}
% \end{figure}
% These two parts are modeled separately for each jet mass .
%
% A three dimensional probability density function for the sub-dominant backgrounds is built as a product of one dimensional pdf's as follows:
% \begin{align}
% P_{Vjets} \left(\MJO, \MJT, \MVV \right) &= P_{dijet} \left( \MVV \right) \times  P_{jet1} \left( \MJO, \MJT \right) \times P_{jet2} \left( \MJT, \MJO \right) \\
...
% \end{align}
% The contribution for the resonant and non-resonant part of the \MJ distributions is modelled separately in order to be able to correlate systematic uncertainties between real vector bosons in these background and the potential signal. $R(\MJ)$ parametrizes the correlation between \MJO and \MJT present for this background and $f$ is a fit parameter that is used to adjust the fraction of real vector boson jets in
% \MJO compared to \MJT. It is fitted within a margin of 10\% around $f=0.5$, which is the value expected for a random sorting of jets.
%
% Since the contribution of the t$\bar{\text{t}}$ background is much smaller than the one coming from V+jets it will be modeled together with the W+jets contribution. All in all two different pdf's are built, one for the W+jets and t$\bar{\text{t}}$ background, and one for Z+jets. This  treatment allows us to fit the normalization for real W jets and real Z jets in data separately.
% Since the statistic available from MC simulation for the high purity category is very low, the parametrization of this background is done only for the low purity category and the results from this are then used for both purity categories. The uncertainties for different categories are however included as separate nuisance parameters in the final fit.
%
%
% For the non-resonant dijet invariant mass shape, the same kernel approach as for the QCD multijet background is used. Since the MC simulations for these minor background contain fewer events than is the case for the QCD multijet backgrounds, the final shapes for high masses are not smooth and have to be put through an additional smoothing procedure. This consists of fitting the tail of the \MVV distribution with a three parameter dijet function
% \begin{equation}
% \frac{\text{d} N}{\text{d} m} = \frac{P_0 (1-m/s)^{P_1}}{(m/s)^{P_2}}~,
% \end{equation}
% where $s$ is the center of mass energy. The function is fitted to the spectrum starting from a dijet mass of 1.1 (2.1)~TeV. The number of events in the final kernel is taken from the dijet function rather than from the smoothing starting from a mass of 1.4 (3.5)~TeV for high purity (low purity). The final 1D kernels are shown in Figure \ref{fig:Vjets_mvv} compared to MC simulation.
%
%
% \begin{figure}[h]
% \centering
% \resizebox{0.49\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debug_mVV_kernels_WJets_HPHP.pdf}}
% \resizebox{0.49\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debug_mVV_kernels_WJets_HPLP.pdf}}\\
...
% \caption{Final one dimensional kernels for the non-resonant background compared to MC simulation for the HPHP category (left) and HPLP (right). The nominal shape derived from the smoothing procedure can be seen as blue line, alternative shapes derived from varying the slope of the \MVV spectrum are shown in green and red. The \MVV kernels for W+jets are shown on top, while the ones for Z+jets are on the bottom.}
% \label{fig:Vjets_mvv}
% \end{figure}
%
%
%
% The alternative shapes used for this background are derived the same way as done for the non-resonant background and are fitted in the final limit setting to accommodate mismodeling of the distribution for example due to higher order QCD and elektroweak corrections which are not considered in the MC simulation. The modeling of the \MJ spectrum is split into two parts, for the resonant and non-resonant part of the spectrum. The resonant events are separated from the non-resonant ones by requiring that there is a generated boson in a cone of $\Delta R = 0.8$ around the reconstructed merged jet. A double-sided crystal-ball function, which is the same shape also used for the modeling of the signal \MJ shapes, is then fitted to the resonant spectrum for W+jets and Z+jets separately. This treatment allows to fully correlate the uncertainties on the mean and width of the \MJ distribution with the signal, since these uncertainties affect all jets generated from real vector bosons in the same way.
%
% In Figure \ref{fig:Vjets_fits} the final fit of a double-sided crystal-ball function to the resonant part of the V+jets spectrum is shown.
%
% \begin{figure}[h]
% \centering
% \resizebox{0.45\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debugJl1_Wjets_TTbar_Res.pdf}}
% \resizebox{0.45\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debugJl2_Wjets_TTbar_Res.pdf}}\\
% \resizebox{0.45\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debugJl1_Zjets_Res.pdf}}
% \resizebox{0.45\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debugJl2_Zjets_Res.pdf}}
...
% \end{figure}
%
% The modeling of the non-resonant part is less critical since this part of the spectrum is very similar to the shapes observed in QCD and can therefore be in parts absorbed into the extensive modelling of the QCD non-resonant backgrounds. For the sake of a better fit stability the non-resonant part of V+jets background is modeled nonetheless using a simple fit with a Gaussian to the non-resonant part of the spectrum as shown in Figure \ref{fig:Vjets_fits_nonRes}.
%
%
% \begin{figure}[h]
% \centering
% \resizebox{0.45\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debugJl1_Wjets_TTbar_nonRes.pdf}}
% \resizebox{0.45\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debugJl2_Wjets_TTbar_nonRes.pdf}}\\
% \resizebox{0.45\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debugJl1_Zjets_nonRes.pdf}}
% \resizebox{0.45\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debugJl2_Zjets_nonRes.pdf}}
...
%
%
% Exhaustive testing shows that correlations between \MJ and the dijet invariant mass are small enough to be neglected in the modeling of these backgrounds, i.e. the shape of the jet mass spectrum does not significantly depend on the momentum of the jets. There is however a correlation between the two jet masses. This correlation stems from the fact that for the V+jets backgrounds one of the two jets in the final state originates from a real boson, while the second is a quark jet. Both of these jets have different distributions which means that depending on the fraction of real boson jets in \MJO the fraction of such jets in \MJT will change. For this reason the fraction real V jets over QCD jets is parametrized as a function of the jet mass. Since a random sorting of jets is used, the parametrization is done once as follows
% \begin{align}
% R(\MJ) &= \frac{N_{res,jet1}(\MJT)+N_{res,jet2}(\MJO)}{N_{non-res,jet1}(\MJT) + N_{non-res,jet2}(\MJO)}~.
% \end{align}
% Where $N(\MJ)$ denotes the number of events within a window of \MJ. The resulting ratio is fitted with a polynomial function as seen in figure \ref{fig:ratio_Vjets}.
%
% \begin{figure}[h]
% \centering
% \resizebox{0.49\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debug_corr_l1_l2_Wjets.pdf}}
% \resizebox{0.49\textwidth}{!}{\includegraphics*{figures/analysis/search3/AN-17-303/resonantBkg/debug_corr_l1_l2_Zjets.pdf}}
% \caption{Parametrization of the ratio of resonant to non-resonant V+jets events given as a function of the jet mass for W+jets plus t$\bar{\text{t}}$ processes (left) and Z+jets processes (right). }
% \label{fig:ratio_Vjets}
% \end{figure}
%
%
% As closure test of the described method the full background model (V+jets contribution and QCD kernels) is fitted to the MC simulation for the minor backgrounds. The results of this fit can be seen in Figure \ref{fig:Vjets_closure} for different slices of \MJ and \MVV projected onto all three jet dimensions. Figure \ref{fig:Vjets_closureHPHP} shows the closure for the high purity category.
%
% \begin{figure}[h]
% \centering
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_X-Proj._y_:_0,-1_z_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Y-Proj._x_:_0,-1_z_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Z-Proj._x_:_0,-1_y_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_X-Proj._y_:_0,-1_z_:_1000,1300}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_X-Proj._y_:_0,-1_z_:_1300,2000}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_X-Proj._y_:_0,-1_z_:_2000,5000}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Y-Proj._x_:_55,65_z_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Y-Proj._x_:_65,85_z_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Y-Proj._x_:_85,105_z_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Y-Proj._x_:_105,215_z_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Z-Proj._x_:_55,65_y_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Z-Proj._x_:_65,85_y_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Z-Proj._x_:_85,105_y_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_Z-Proj._x_:_105,215_y_:_0,-1}.pdf}}
% \caption{Final fits of the complete background model (red line) to the MC simulation of the V+jets backgrounds (black markers) for the low purity category.}
% \label{fig:Vjets_closure}
% \end{figure}
%
%
% \begin{figure}[h]
% \centering
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_HPHP_X-Proj._y_:_0,-1_z_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_HPHP_Y-Proj._x_:_0,-1_z_:_0,-1}.pdf}}
% \resizebox{0.3\textwidth}{!}{\includegraphics*{{figures/analysis/search3/AN-17-303/resonantBkg/PostFitVjetsMC_HPHP_Z-Proj._x_:_0,-1_y_:_0,-1}.pdf}}\\
